...
\begin{document}
	\twocolumn[{\LARGE\centering CUCaTS Python Cheatsheet \\[2\baselineskip]}]
	...
	Labels onto values. Their names must consist only of letters, digits and underscores. They cannot start with a digit or have spaces. They are case sensitive.
	...
	\begin{tabular}{l l}
		Element access &
		\colorbox{codebg}{\lstinline|$\colorbox{spacebg}{\quad}$[$\colorbox{lightspacebg}{index}$]|} \vspace{1mm} \\
		
		Slicing &
		\colorbox{codebg}{\lstinline|$\colorbox{spacebg}{\quad}$[$\colorbox{lightspacebg}{start}$:$\colorbox{lightspacebg}{end}$]|} \vspace{1mm} \\
		
		Comprehensions &
		\colorbox{codebg}{\lstinline|[$\colorbox{spacebg}{\quad}$ for $\ \colorbox{spacebg}{\quad}$ in $\ \colorbox{spacebg}{\quad}$]|} \vspace{1mm} \\
		
		Length &
		\colorbox{codebg}{\lstinline|len($\colorbox{spacebg}{\quad}$)|}
	\end{tabular}
