...
\begin{document}
	...
	\begin{frame}[fragile]
		...
		\begin{itemize}
			\item Run a block of code for every element in a list.
			\item Indentation specifies the block.
			\item `\lstinline|for|' and `\lstinline|in|' are keywords.
			\note<1->{Keywords are ones that have a special meaning. You cannot use them as variable names. List comprehensions must use `\lstinline|for|' and `\lstinline|in|'}
		\end{itemize}
		...
		\begin{columns}[c]
			...
			>>> for num in numbers:
			...     total = total + num
			$\note<1>{Take every element of \lstinline|numbers| in turn and call it \lstinline|num|.}
	...
	\begin{frame}[fragile]
		...
		A function is a reusable piece of code. It takes an input value (\evenbox{jargonbg}{parameter}), processes it and gives back an output (\evenbox{jargonbg}{return value}). \pause
