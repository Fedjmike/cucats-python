...
\definecolor{jargonbg}{rgb}{0.5,1.0,0.8}
...
\lstdefinelanguage{py}{
	morekeywords={if,elif,else,for,in,def,class,True,False},
	literate=
		*{=}{{{\color{punct}{=}}}}{1}
		{|}{{{\color{punct}{|}}}}{1}
		{\^}{{{\color{punct}{\^{}}}}}{1}
...
\begin{frame}[fragile]
	...
	\item Things you do in IDLE aren't permament. You can always close IDLE and open it afresh.
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Dictionaries % bother explaining them this early?
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{onlyenv}<3>
				\begin{lstlisting}
				>>> 9.0[1]
				$\textrm{\color{red}TypeError: `int' object has no attribute `\_\_getitem\_\_'}$
				\end{lstlisting}
			\end{onlyenv}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\note<1>{Variable names must consist only of letters, digits and underscores. They can't start with a digit. They are case sensitive, }
	\end{itemize}
...
\begin{frame}[fragile]
	...
	% restate a former for+if example more concisely, teach them to think in terms of transforming data, pipelines
\end{frame}
